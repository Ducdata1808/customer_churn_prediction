...
export default function LandingPage() {
  ...
  return (
    <div className="relative h-screen w-screen bg-[#F2F0E9] dark:bg-[#0D0D12] text-[#1A1A1A] dark:text-[#FAF8F5] overflow-hidden">
      ...
      <nav className={`fixed top-6 left-1/2 -translate-x-1/2 z-50 w-[90%] max-w-5xl rounded-full border border-[#2E4036]/10 dark:border-[#FAF8F5]/10 px-6 py-3 flex items-center justify-between transition-all duration-500 ${activeSection > 0
        ...
        <div className={`hidden md:flex items-center gap-8 font-heading text-sm font-semibold ${activeSection === 0 ? "text-white/80" : "text-[#1A1A1A]/80 dark:text-[#FAF8F5]/80"
          }`}>
                  ...
                  <div className="p-3 bg-[#2E4036]/5 dark:bg-white/5 rounded-xl border border-[#2E4036]/5 dark:border-white/5 transition-colors duration-500">
                    ...
                    <div className="grid grid-cols-3 gap-1.5">
                      {["id", "tenure", "MonthlyCharges", "TotalCharges", "Contract", "Churn"].map((col) => {
                        ...
                        return (
                          <div key={col} className={`flex items-center gap-1 p-1 rounded-lg border text-[9px] font-data transition-all duration-300 ${isValid ? "bg-emerald-500/10 border-emerald-500/20 text-emerald-600 dark:text-emerald-400 font-bold" : "bg-[#2E4036]/5 dark:bg-white/5 border-[#2E4036]/5 dark:border-white/5 text-[#1A1A1A]/80 dark:text-white/80"
                            }`}>
                  ...
                  <div className="grid grid-cols-3 gap-1.5">
                    {Object.keys(schemaDetails).map((col) => (
                      <button
                        ...
                        className={`px-2 py-1 rounded-xl text-[9px] font-data border transition-all duration-300 truncate ${selectedSchemaCol === col
              ...
              >
                <div style={{ transform: "translateZ(15px)" }}>
                  <div className="flex items-center justify-between border-b border-white/10 pb-2.5 mb-2.5">
                    ...
                    <span className={`text-[8px] uppercase font-bold px-2 py-0.5 rounded ${ingestState === "completed" ? "bg-[#CC5833]/15 text-[#CC5833] border border-[#CC5833]/30" : "bg-yellow-500/15 text-yellow-400"
                  ...
                  <div className="flex gap-3 text-[10px] uppercase font-bold">
                    {["metrics", "matrix", "roc"].map((tab) => (
                      <button
                        ...
                        className={`pb-2 px-1 transition-all ${activeTab === tab
